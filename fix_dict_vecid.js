...
// Fix processDictionaryIncrementalZh: add setVecId + saveAll after batchInsert
// Pattern: zvecBridge.batchInsert(collection, batchDocs, vectors)\n                            totalProcessed += batchDocs.size\n                            moduleCount += batchDocs.size
// There are two occurrences (batch >= 100 and remaining)

// Fix 1: batch >= 100 case
const old1 = `zvecBridge.batchInsert(collection, batchDocs, vectors)
                            totalProcessed += batchDocs.size
                            moduleCount += batchDocs.size
                        } catch (e: Exception) {
...
const new1 = `zvecBridge.batchInsert(collection, batchDocs, vectors)
...
const old2 = `zvecBridge.batchInsert(collection, batchDocs, vectors)
                        totalProcessed += batchDocs.size
                        moduleCount += batchDocs.size
                    } catch (e: Exception) {
                        totalErrors += batchDocs.size
...
const new2 = `zvecBridge.batchInsert(collection, batchDocs, vectors)
                        batchDocs.forEach { doc -> doc.setVecId("tfidf_256", "dictionary_\\${doc.sourceRef}_\\${doc.chunkIndex}") }
                        docRepo.saveAll(batchDocs)
                        totalProcessed += batchDocs.size
                        moduleCount += batchDocs.size
