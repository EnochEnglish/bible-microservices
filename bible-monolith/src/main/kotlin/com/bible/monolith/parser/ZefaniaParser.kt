 ...
 * Zefania XML 格式解析器
 *
 * Zefania 是另一种常见的圣经 XML 格式
 * open-bibles 项目中 ASV、DARBY、DRA、KJV 等使用此格式
 *
 * Zefania XML 结构示例：
 * <XMLBIBLE>
 ...
 */
@Component
class ZefaniaParser : BibleParser {
    override val format = "zefania"
    private val logger = LoggerFactory.getLogger(ZefaniaParser::class.java)

    /**
     * Zefania 格式与 USFX 格式结构高度相似
     * 主要区别在根元素和元数据位置
     */
    override fun parse(content: String): ParseResult {
        ...
        val translationMeta = TranslationMeta(
            identifier = doc.documentElement.getAttribute("biblename")?.takeIf { it.isNotBlank() } ?: "unknown",
            name = doc.documentElement.getAttribute("biblename") ?: "Unknown",
            language = "",
            languageCode = "",
