...
import com.bible.monolith.model.Word
...
import com.bible.monolith.repository.WordRepository
...
class BibleTextService(
     ...
     */
    @Transactional(readOnly = true)
    fun getInterlinear(translation: String, book: String, chapter: Int): Map<String, Any> {
        val bookEntity = getBookEntity(translation, book)
            ?: return errorResult("not_found", "书卷不存在: $book")

        val verses = verseRepository.findByBookAndChapterOrderByVerse(bookEntity, chapter)
        if (verses.isEmpty()) {
            return errorResult("not_found", "章节不存在: $book $chapter")
        }

        val verseIds = verses.mapNotNull { it.id }
        val words = wordRepository.findByVerseIdInOrderByPosition(verseIds)
        val wordsByVerseId = words.groupBy { it.verse.id }

        val versesData = verses.map { v ->
            val verseWords = wordsByVerseId[v.id] ?: emptyList()
            mapOf(
                "chapter" to v.chapter,
                "verse" to v.verse,
                "text" to v.text,
                "words" to verseWords.map { w ->
                    mapOf(
                        "position" to w.position,
                        "text" to w.text,
                        "strongs" to (w.strongs ?: ""),
                        "lemma" to (w.lemma ?: ""),
                        "morphology" to (w.morphology ?: ""),
                        "is_parenthetical" to w.isParenthetical
                    )
                }
            )
        }
     ...
     */
    @Transactional(readOnly = true)
    fun getInterlinearVerse(translation: String, book: String, chapter: Int, verse: Int): Map<String, Any> {
        val bookEntity = getBookEntity(translation, book)
            ?: return errorResult("not_found", "书卷不存在: $book")

        val verseEntity = verseRepository.findByBookAndChapterAndVerse(bookEntity, chapter, verse)
            ?: return errorResult("not_found", "经文不存在: $book $chapter:$verse")

        val words = wordRepository.findByVerseIdOrderByPosition(verseEntity.id!!)
